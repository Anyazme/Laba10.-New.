...
namespace newlaba10
{
	internal class Program
	{
		static void Main(string[] args)
		{
			...
			//создание массива интерфейса IInit
			Console.WriteLine("Библиотека классов:");
			IInit[] arr = new IInit[25];

			Random random = new Random();

			//заполнение массива объектами через случайные значения
			for (int i = 0; i < 5; i++)
			{
				arr[i] = new Aircraft();
				arr[i].RandomInit();
			...
			//подсчет объектов классов в массиве
			int AircraftCount = arr.Count(obj => obj is Aircraft);
			int HelicopterCount = arr.Count(obj => obj is Helicopter);
			int PlaneCount = arr.Count(obj => obj is Plane);
			int FighterCount = arr.Count(obj => obj is Fighter);
			int RunnerCount = arr.Count(obj => obj is Runner);

			Console.WriteLine($"Количество воздушных суден: {AircraftCount}, \nКоличество вертолетов: {HelicopterCount}, \nКоличество самолетов:{PlaneCount}, \nКоличество истребителей: {FighterCount}, \nКоличество бегунов: {RunnerCount} ");

			//заданный класс истребителя и количество экипажа по этому классу
			...
			//все воздушные судна, произведенные после 2020
			AircraftAfter2020(arr);

			//модели с макс дальностью полета
